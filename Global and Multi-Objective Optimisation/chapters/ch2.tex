...
\begin{itemize}
    \item \textbf{Population size} $N$: Number of individuals maintained at each generation (tipically 100-200). Larger populations increase diversity and exploration but require more computational resources.
...
\begin{enumerate}
    \item Two crossover points are selected in the parent permutations
    \item A mapping is constructed between the segments defined by these points
    \item The segments are exchanged between parents
    \item Any conflicts are resolved by iteratively applying the mapping until a valid permutation is obtained
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Adjacency Matrix:} A binary matrix indicates the presence or absence of edges between vertices
    \item \textbf{Edge List:} Explicitly evolve sets of vertices $V$ and edges $E$
\end{itemize}
