...
\begin{itemize}
    \item The population is a single set of rules.
    ...
    \item It includes a \bfit{covering} mechanism: if the agent encounters a state $s$ for which no rule matches, a new rule is created that covers $s$ (with a random action). This new rule replaces an existing, low-fitness rule in the population.
\end{itemize}

\paragraph{ZCS Algorithm Cycle}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{assets/zcs-cycle.png}
...
\paragraph{Fitness Update Mechanism}
...
\begin{enumerate}
    ...
    \item Select one rule from $M$ to apply (e.g., via fitness-proportional selection) and extract its action $a$.
    ...
    \item The fitness of each rule $A_i \in A$ is updated by redistributing payoff. Each rule gives a fraction of its fitness to the rules in the *previous* action set, and in return receives a share of the external reward $r$ plus the discounted value of the next action set $A'$:
    $$ \text{Fitness}(A_i) \leftarrow (1-\alpha)\text{Fitness}(A_i) + \alpha \frac{1}{|A|} \left( r + \gamma \sum_{A'_j \in A'} \text{Fitness}(A'_j) \right) $$
    \item Additionally, rules in $M$ that proposed different actions ($B=M-A$) are penalized by having their fitness reduced: $\text{Fitness}(B_i) \leftarrow \beta \cdot \text{Fitness}(B_i)$, with $\beta \in [0,1]$.
\end{enumerate}

\paragraph{Fitness Redistribution in the GA}
When the GA is triggered, two parent rules $P_1, P_2$ are selected. To conserve fitness within the system:
\begin{itemize}
    \item If no crossover occurs, the parents and children ($C_1, C_2$) share fitness: $\text{Fitness}(C_i) \leftarrow \frac{1}{2}\text{Fitness}(P_i)$ and $\text{Fitness}(P_i) \leftarrow \frac{1}{2}\text{Fitness}(P_i)$.
    \item If crossover occurs, children receive a portion of the parents' average fitness, and the parents' fitness is halved: $\text{Fitness}(C_i) \leftarrow \frac{1}{4}(\text{Fitness}(P_1) + \text{Fitness}(P_2))$.
\end{itemize}
...
\paragraph{Accuracy-Based Fitness}
The central innovation in XCS is that a rule's fitness is based on the \bfit{accuracy} of its prediction, not the magnitude of the prediction itself. This encourages the system to form a complete and accurate map of the entire state-action-payoff landscape, rewarding rules that are reliable predictors, even if they predict low payoffs. XCS maintains several distinct measures for each rule:
\begin{enumerate}
