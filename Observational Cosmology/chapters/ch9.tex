...
\begin{observationblock}[Repeatability]
        Real cosmological data are not drawn from repeated universes. This doesn't invalidate frequentist methods, but it means probability statements about \emph{parameters} are usually made indirectly (e.g., confidence intervals, $p$-values) rather than as direct ``degrees of belief.''
   
\end{observationblock}

In practice, \textbf{Bayesian methods} dominate parameter estimation in cosmology because they combine heterogeneous experiments naturally (via hierarchical models), propagate nuisance parameters cleanly, and deliver interpretable uncertainty regions for physical quantities.
...
\begin{minipage}{0.48\textwidth}
    \textbf{Frequentist workflow}
    \begin{itemize}[noitemsep]
        \item Gives probabilities of hypotheses (via $p$-values), not of models themselves.
        \item Emphasis on likelihood-based estimators and calibrated coverage.
        \item No prior over parameters is required.
        \item Still common in particle physics (e.g.\ hypothesis tests with $p$-values).
    \end{itemize}
...
\begin{definitionblock}[Bayes' theorem (parameter inference)]
    ...
    \begin{itemize}
        \item $p(\theta \mid d, M)$ is the \textbf{posterior} (state of knowledge about parameters after the measurement);
        \item $P(d \mid \theta, M)$ is the \textbf{likelihood} ($\mathcal{L}$) (probability density assigned to the data for fixed $\theta$);
        \item $P(\theta \mid M)$ is the \textbf{prior} ($\pi$) (density encoding information before the experiment);
        \item $P(d \mid M) = \int P(d \mid \theta, M)\,P(\theta \mid M)\,\dd{\theta}$ is the \textbf{evidence} ($\mathcal{Z}$) (normalization constant).
    \end{itemize}
\end{definitionblock}
...
\textbf{Priors} quantify what is known about the parameters before using the data from your specific experiment. This includes:
\begin{itemize}
    \item \textbf{Theoretical limits}, physical boundaries (such as $\Omega_m > 0$), and requirements for model stability;
    \item Preferences or \textbf{constraints} derived from previous, simpler data or established theory (e.g., hierarchical priors).
\end{itemize}

In regions where the likelihood is strictly zero, the choice of prior does not impact the resulting posterior for parameter estimation. However, it \emph{does} still matter when performing more advanced tasks like Bayesian model selection.

It is common in cosmology to use \textbf{uniform priors} (or \textbf{flat priors}) for most parameters, aiming to assign equal probability to "equal states of knowledge." Yet, flat priors are not always as harmless as they seem: the main subtlety is that a flat prior on a parameter $\theta$ does \emph{not} generally induce a flat prior on a non-linear function of that parameter, $\psi(\theta)$. Flatness is not invariant under reparameterization.
If $\psi = g(\theta)$ is an invertible transformation, conservation of probability imposes that the priors are related by:
$$
p(\psi) = p(\theta)\left|\frac{\mathrm{d}\theta}{\mathrm{d}\psi}\right|
$$
Thus, a uniform prior on $\theta$ almost always induces a \emph{non-uniform} prior on $\psi = g(\theta)$, unless $g$ is linear. The Jacobian shows why ``uninformative'' priors are a myth: any prior imposes a measure on parameter space, and careless choices can distort results. E.g., uniform priors on $\Omega_m$ and $h$ produce a strongly non-uniform prior on $\Omega_m h^2$, even if that combination is actually measured by the data.

\medskip

\begin{minipage}{0.7\textwidth}
    \includegraphics[width=\textwidth]{assets/priors_ex.png}
\end{minipage}%
\hfill
\begin{minipage}{0.28\textwidth}
    \captionof{figure}{Jointly uniform priors on $\Omega_m$ and $h$ (left) map onto a strongly non-uniform prior density in the $\Omega_m h^2$-$h$ plane (right).}
    \label{fig:priors_jacobian}
\end{minipage}

\subsubsection{Example: priors on dynamical dark energy}

Broad uniform priors on parametrizations such as the CPL pair $(w_0, w_a)$ allocate substantial prior mass to regions that are physically extreme or poorly constrained by the data. Posteriors and tensions can then shift noticeably when one instead uses priors motivated by scalar-field potentials (``hilltop'', ``exponential'', etc.), even when the likelihood is unchanged. Toomey et al.\ (2025), for instance, find that a joint fit of Planck PR4 CMB anisotropies with DESI DR2 BAO prefers evolving dark energy over $\Lambda$CDM at the $\sim 3.1\sigma$ level under broad uniform priors on $(w_0, w_a)$, a preference that drops to only $\sim 1.3\sigma$ (hilltop prior) or $\sim 1.8\sigma$ (exponential prior) once theoretically motivated priors are adopted instead, with the likelihood itself left completely unchanged.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.45\textwidth}
        \includegraphics[width=0.95\textwidth]{assets/priors.png}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.5\textwidth}
        \captionof{figure}{Two-dimensional posteriors for $(w_0, w_a)$ at 68\% and 95\% credible levels (Planck PR4 + DESI DR2), illustrating prior dependence for evolving dark energy models. Gray dashed lines mark the $\Lambda$CDM point $(w_0, w_a) = (-1, 0)$. Theory-motivated priors (blue: hilltop; red: exponential) pull the distribution toward $\Lambda$CDM compared with a very broad uniform prior (black dashed). The lesson is operational: always document priors and, when possible, check sensitivity.}
...
For models with even a few dozen parameters, exploring the parameter space using a regular grid becomes computationally impossible, requiring many thousands of years.

Suppose a model has just $10$ free parameters, that a single likelihood evaluation takes $1$ second, and that each parameter is sampled on a grid of $20$ values. The total number of grid points is $N = 20^{10} \simeq 10^{13}$ so that evaluating the likelihood once at every point would take roughly $3\times10^5$ years (some 300{,}000 years), which is obviously infeasible even for such a modest parameter count. Any realistic cosmological analysis, with a comparable or larger number of parameters, is therefore completely out of reach of brute-force grid evaluation.

\smallskip

As a result, modern cosmology relies primarily on two major classes of methods:
\begin{itemize}
    \item \textbf{Monte Carlo Markov Chain (MCMC) 
    methods:} MCMC algorithms construct a 
    chain of samples so that, after an initial 
...
\begin{tipsblock}[Choosing a sampler]
No single algorithm is optimal for every problem: the right choice depends on the dimensionality, the shape of the posterior (multimodality, strong degeneracies), and whether the evidence itself is needed. Fowlie et al.\ provide a systematic comparison of MCMC and nested-sampling performance across representative cosmological problems, while Lemos et al.\ (2022) focus specifically on comparing different nested-sampling implementations.
\end{tipsblock}
...
The detailed-balance condition ensures that this acceptance rule leads the Markov chain to sample from the true posterior, regardless of the starting point. In practice, however, the speed of convergence depends sensitively on the choice of proposal distribution.

Poorly tuned proposals slow exploration: small steps result in high acceptance but inefficient, slow-moving chains, while large steps produce low acceptance and correlated samples. Affine-invariant samplers (e.g.\ \textsc{emcee}) help by adapting to the posterior's geometry using an ensemble of walkers.



\subsubsection{Burn-in and convergence diagnostics}

Early steps of a chain typically reflect initial conditions rather than the target posterior, so a \textbf{burn-in} (or \textbf{warm-up}) phase is discarded before analysis, there is no fixed fraction; the required length depends on the problem's specifics. A well-mixed MH chain should appear as white noise when plotting a parameter versus step number. Two common issues are easily seen: long periods of stuck samples and low acceptance indicate the proposal width is too large; erratic wandering with high acceptance points to it being too small.
...
\begin{center}
    ...
    \captionof{figure}{Example of MCMC chains converging after an initial burn-in phase.}
    \label{fig:burn_in}
\end{center}
...
\begin{minipage}{0.66\textwidth}
    \begin{algorithm}[H]
        ...
        \begin{algorithmic}[1]
            ...
            \While{stopping criterion on remaining evidence is not met}
                \State Find the point $\theta_i$ with smallest likelihood $L_i$
                \State Replace $\theta_i$ with a new point from $\pi(\theta)$ s.t. $L(\theta) > L_i$
                \State Record the \textit{dead point} and its likelihood
            \EndWhile
            \State \textbf{Output:} posterior samples and accumulated evidence.
        \end{algorithmic}
...
\begin{minipage}{0.33\textwidth}
    \centering
    \includegraphics[width=\textwidth]{assets/nested-sampling.png}

    \vspace{-0.5em}

    \captionof{figure}{Nested sampling}
    \label{fig:nested_sampling}
\end{minipage}

\medskip

The same operation, applied to a single column of the weighted sample, yields any lower-dimensional marginal: to obtain the posterior for one parameter marginalized over all the others, it suffices to histogram that parameter's values against the same weights.
...
In practice, $\nu_{\text{eff}}$ can also be estimated empirically, without relying on any of the three formulas above: one generates a large number of mock data realizations from the best-fit model, refits each realization to recover its own best-fit $\chi^2$, and builds up the resulting distribution of $\chi^2_{\text{BF}}$ values. Fitting this distribution to the functional form expected for a $\chi^2$ distribution with $\nu$ degrees of freedom then returns both a point estimate of $\nu_{\text{eff}}$ and its uncertainty (e.g.\ $\nu_{\text{eff}} = 18.65 \pm 0.59$ in a representative cosmic-shear analysis); this effective number typically comes out somewhat below the nominal parameter count, reflecting the informative priors and parameter correlations present in the fit.
...
There is no unique "metric" to assess the level of tension or agreement between data sets, and several approaches have been developed (Lemos et al.\ 2020; see also e.g.\ Grandis et al.\ 2016, Charnock et al.\ 2017, and Raveri et al.\ 2020), which can be broadly split into two categories:
...
\begin{itemize}
    ...
    The Bayes Ratio $R$ is defined for independent datasets $A$ and $B$, and for their combination $AB$, as:
    $$
    R \equiv \dfrac{\mathcal{Z}_{AB}}{\mathcal{Z}_{A}\mathcal{Z}_{B}}
    \qquad
    \text{where}
    ...
    Smaller values of $R$ indicate stronger evidence of tension between datasets $A$ and $B$; the customary interpretation of $\ln R$ follows \bfit{Jeffreys' scale}:
    \begin{center}
        \small
        \begin{tabular}{ll}
            \toprule
            \textbf{$\ln R$} & \textbf{Interpretation} \\
            \midrule
        ...
        \end{tabular}
    \end{center}
    Because $R$ requires the full evidence integral, its value depends explicitly on the assumed prior volume: simply widening or narrowing the prior range of any parameter can shift $\ln R$ substantially even though the likelihood, and hence the data themselves, are entirely unchanged (see the worked example in \cref{subsec:model_selection}).
...
For a recent worked application of these tension metrics to real cosmological data sets, see e.g.\ arXiv:2511.04661.

\subsection{Model Selection}
\label{subsec:model_selection}

Model selection is the process of choosing the best model from a set of candidates. To determine which model is preferred by a given data set, a simple comparison of $\chi^2$ values might not be sufficient, for instance when the competing models have a different number of free parameters or are analysed under different priors, so that a lower $\chi^2$ may simply reflect extra flexibility (overfitting) rather than a genuinely better description of the data. Two widely used techniques that address this are the \bfit{Bayes Evidence Ratio} and the \bfit{Deviance Information Criterion}.
...
A simple illustration of this Occam's-razor behaviour is fitting polynomials of increasing degree to the same noisy data set: adding more terms can only reduce (or leave unchanged) the best-fit $\chi^2$, yet the Bayesian evidence typically peaks at some intermediate polynomial order and falls again once additional terms inflate the prior volume without a matching gain in likelihood, automatically penalising unnecessary model complexity.
...
The model with the lower DIC either fits better the data (lower $\langle \chi^2\rangle$) or has a lower level of complexity (lower $\langle \chi^2\rangle - \chi^2_{MaxP}$).

DIC can be easily computed directly from the parameters posteriors, but it is only strictly valid when the posterior distribution is well approximated by a multivariate Normal distribution.

\begin{exampleblock}[Prior dependence - DESI]
Consider testing $\Lambda$CDM (model 1) against a model with extra free parameters, such as $w_0w_a$CDM (model 2), where the additional parameters are assigned uniform priors of width $d_{\nu_i}$, $\mathcal{U}(\nu_i) = 1/d_{\nu_i}$. Writing the (constant) prior density of each model as $\mathcal{P}_i = \prod_k 1/d_{\nu_k,i}$, the Bayes factor comparing the two models is
$$
B_{21} = \frac{\mathcal{P}_2\int\mathcal{L}_2\,d\theta}{\mathcal{P}_1\int\mathcal{L}_1\,d\theta} = \frac{\mathcal{P}_2}{\mathcal{P}_1}\,\Gamma = \frac{(d_{\nu_1}\,d_{\nu_2}\,d_{\nu_3}\dots)_1}{(d_{\nu_1}\,d_{\nu_2}\,d_{\nu_3}\dots)_2}\,\Gamma ,
$$
where $\Gamma$ is the ratio of the plain likelihood integrals and the prefactor is the ratio of prior volumes: extra parameters with wide priors suppress $\mathcal{P}_2$ and hence penalise the more complex model, exactly the Occam factor introduced above. If the prior range of every extra parameter of model 2 is instead rescaled by a common factor $\alpha$, $d^\star = \alpha\,d$, the Bayes factor shifts by
$$
\log B_{21}^\star = \log\!\left[\frac{(\alpha_{\nu_1}\,\alpha_{\nu_2}\,\alpha_{\nu_3}\dots)_1}{(\alpha_{\nu_1}\,\alpha_{\nu_2}\,\alpha_{\nu_3}\dots)_2}\right] + \log B_{21} .
$$
Applying this to the DESI DR2 BAO analysis (jointly fit with the CMB and one of three SN Ia compilations, PantheonPlus, Union3, or DES Y5), simply widening the prior range on $(w_0,w_a)$ by a factor $\alpha$ from $1$ to $60$ pushes $\log B_{12}$ (the evidence for $\Lambda$CDM over $w_0w_a$CDM) from close to the ``weak''/``positive'' boundary up into the ``strong'' regime on Jeffreys' scale, for every SN compilation considered. Since neither the data nor the likelihood have changed, this drift is entirely a consequence of the (seemingly innocuous) choice of prior range, precisely the caveat already noted for the Bayes Ratio above.
\end{exampleblock}
