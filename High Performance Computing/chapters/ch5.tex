...
\subsubsection{Other to know about OpenMP}
...
\begin{observationblock}[\plaintt{\_OPENMP}]
    The macro \plaintt{\_OPENMP} is automatically defined by the compiler when OpenMP support is enabled (e.g., with \plaintt{-fopenmp}). You can use it to conditionally compile code that should only be included when OpenMP is available.
    ...
    \begin{codeblock}[language=C]
#ifdef _OPENMP
    // OpenMP-specific code here
#else
    // Fallback or serial code here
#endif
    \end{codeblock}
\end{observationblock}
