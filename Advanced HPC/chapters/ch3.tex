...
\begin{tipsblock}[OpenMP 5.x Best Practices]

Recommendation hierarchy:

\begin{enumerate}
    \item \textbf{First choice}: High-level synchronization constructs
    \begin{itemize}
        \item \plaintt{\#pragma omp barrier}
        \item \plaintt{\#pragma omp critical}
        \item \plaintt{\#pragma omp atomic} with appropriate memory order
    \end{itemize}

    \item \textbf{Second choice}: Atomics with explicit memory order
    \begin{itemize}
        \item Use \plaintt{acquire} for reads in consumers
        \item Use \plaintt{release} for writes in producers
        \item Use \plaintt{relaxed} for true race-free counters
    \end{itemize}

    \item \textbf{Last resort}: Explicit flush
    \begin{itemize}
        \item Only when you need to synchronize multiple variables simultaneously
        \item Only when atomics are not available (old OpenMP versions)
        \item Document extensively why flush is necessary
    \end{itemize}

    \item \textbf{Never}: \plaintt{volatile} in OpenMP code
    \begin{itemize}
        \item Provides no synchronization guarantees
        \item Can prevent compiler optimizations
        \item Use atomics instead
    \end{itemize}
\end{enumerate}
\end{tipsblock}