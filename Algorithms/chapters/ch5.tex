...
\section{Naive Algorithm (Shifts Version)}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State $k \gets 0$ \Comment{length of the current lowest prefix}
        \For{$q \gets 2$ to $m$}\Comment{length of the substring we are currently checking}
            \While{$k > 0$ and $P[k + 1] \neq P[q]$}
                \State $k \gets \pi[k]$
            \EndWhile
            \If{$P[k + 1] = P[q]$}
    ...
    \begin{algorithmic}[1]
        ...
        \For{$i = 1$ \textbf{ to } $n$ \textbf{ do}} 
            ...
            \If{$P[q + 1] = T[i]$} \Comment{Next character matches}
                \State $q \gets q + 1$
            \EndIf
            \If{$q = m$} \Comment{Found complete pattern match}
                \State $solution.append(i - m + 1)$
