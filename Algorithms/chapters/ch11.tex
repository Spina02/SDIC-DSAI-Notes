...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{assets/discrete-convolution.png}
    \vspace{1em}
    \caption{Data flow for the discrete convolution of two vectors of size 3 on a linear array.}
    \label{fig:discrete-convolution}
\end{figure}

\section{Discrete Fourier Transform}

The \textbf{Discrete Fourier Transform (DFT)} of a vector $\vec{x}$ of size $N$ is the vector of the same size defined by
$$
    \hat{x}_k = \sum_{i=0}^{N-1} x_i\, \omega^{i\cdot k} \quad \text{for every } 0 \leq k \leq N-1,
$$
where $\omega$ is a primitive $N$-th root of unity. The DFT has many uses in different fields. Among them, it can be used, along with its inverse operation (IDFT), to compute the discrete convolution.
...
\begin{itemize}[noitemsep]
    \item to store $\omega$,
    \item to accumulate the sum $\tilde{x}_k$, and
    \item to store a value $v_k$ corresponding to a power of $\omega$ depending on the current step of the sum.
\end{itemize}

The value $v_k$ is not to be confused with the step of the execution.

We can assume that each processor has already stored $\omega$ in its first memory cell. If this were not the case, we could easily prepend $\omega$ to the input stream and have each processor store its first received value as $\omega$. Additionally, we can initialize each processor $k$ with $v_k = \omega^{-1}$ in its third memory cell and $\hat{x}_k = 0$ in its second memory cell.
...
\begin{enumerate}
    \item After processing unit $k$ has received the $i$-th value $x_i$ and has updated its stored values, its $v_k = \omega^{k}$, hence it will relay the product $x_i \cdot \omega^{k}$.
    \item At each step, the value $x_i$ is multiplied by the appropriate power of $\omega$ as it moves through the array, so that when it reaches processor $k$, it is multiplied by $\omega^{i\cdot k}$.
\end{enumerate}
...
\begin{itemize}
    \item The \textbf{parallel execution time} is $T_N(N) = \Theta(N)$, since each of the $N$ values must traverse the array, and each processor performs $N$ constant-time operations.
    \item The \textbf{work} is $W = \Theta(N^2)$, as each of the $N$ processors performs $N$ operations.
    \item The \textbf{speedup} is $S = \Theta(N^2)/\Theta(N \log N) = \Theta(N/\log N)$ with respect to the fastest sequential FFT algorithm, and the efficiency is $\mathcal{E} = \Theta\left(\frac{1}{\log N}\right)$.
\end{itemize}

\newpage

\begin{exampleblock}[DFT computation on a linear array]
    ...
    \begin{tabular}{|c||c|c|c|c|c|c|}
        ...
        \hline
        \hline
        $\hat{x}_0$ & \makebox[1cm]{0} & \makebox[1cm]{$x_0$} & $x_0 + x_1$ & $x_0 + x_1 + x_2$ & $x_0 + x_1 + x_2$ & $x_0 + x_1 + x_2$ \\
        ...
        \hline
        \hline
        $v_0$ & $\omega^{-1}$ & 1 & $\omega$ & $\omega^2$ & $\omega^2$ & $\omega^2$ \\
        $v_1$ & $\omega^{-1}$ & $\omega^{-1}$ & 1 & $\omega$ & $\omega^2$ & $\omega^2$ \\
        $v_2$ & $\omega^{-1}$ & $\omega^{-1}$ & $\omega^{-1}$ & 1 & $\omega$ & $\omega^2$ \\
        \hline
    \end{tabular}
    \end{center}
    ...
    We can observe how each processor accumulates its partial sum over time. For instance, processor 0 computes $\hat{x}_0$ by simply adding all input values, while processor 1 multiplies each input by increasing powers of $\omega$ before adding them.
\end{exampleblock}