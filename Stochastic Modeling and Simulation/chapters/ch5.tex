...
\subsubsection{$2^{nd}$ Recipe}

The following family of SDEs:
$$
\begin{cases}
\dot{y}(t) = f(y) + g(y)\xi(t)\\
g(\pm 1) = 0\\
f(1) < 0 \qquad f(-1) > 0\\
f(-y) = -f(y) \qquad g(y) > 0
\end{cases}
$$
defines a family of bounded noises. Indeed suppose that a time $\bar{t}$ it is $y(\bar{t}) = 1$ then
$$
...
$$
with $\delta > -1$ defines a family of bounded noises called the Doering-Cai-Lin noise, whose stationary distribution is
$$
P_s(y) = A(1 - y^2)^{\delta}
$$
Thus, if $\delta < 0$ then the stationary PDF is 'horn-shaped' bimodal and if $\delta > 0$ is unimodal.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{assets/doering-cai-lin.png}
    \caption{Doering-Cai-Lin noise for different values of $\delta$.}
\end{figure}

\subsubsection{$3^{rd}$ Recipe}

Let us consider a material point of mass $m = 1$ that moves in 1D under the action of three forces: a damping $F_d = -\gamma v$, a stochastic force $\omega \xi(t)$ and a force $F(y) = -U'(y)$ such that
$$
...
It follows that the point motion
$$
\dot v = -\gamma v + F(y) + \omega \xi(t)
$$
is constrained by a infinite potential well so that:
$$
...
% \subsection{Important Considerations and Paradoxes}

% \subsubsection{The Malthusian Growth Exception}

% \begin{warningblock}[Attention!]
% If a parameter can be negative, then there is no problem in using unbounded noises.

% \textbf{Example: the Malthusian growth}
% $$
% \dot{x} = rx
% $$
% The net birth rate $r$ is in reality the result of the difference:
% $$
% r = b - m
% $$
% If both fluctuate, the number of deaths per time unit can be larger than the number of births per time unit!

% No problems! So we can safely set:
% $$
% r + \omega\xi(t)
% $$
% \end{warningblock}

% This shows that the need for bounded noise depends critically on the physical interpretation and constraints of the parameters in question.

% \subsubsection{The Tumor Therapy Paradox}

% Consider a model of tumor under constant continuous therapy:
% $$
% \dot{x} = f(x)x - \theta x
% $$
% where $\theta$ represents the therapy intensity. Perfect constant therapy does not exist due to fluctuations:
% $$
% \dot{x} = f(x)x - \theta(1 + \omega\xi(t))x
% $$

% In Ito form:
% $$
% dx = f(x)xdt - \theta(dt + \omega dW)x
% $$

% \textbf{PARADOX:} Cells killed per time unit are represented by:
% $$
% \theta(dt + \omega dW)x
% $$

% However, according to d'Onofrio (2008):
% $$
% \text{Prob}(dt + \omega dW < 0) > 0
% $$

% This model allows adding tumor cells to the patient! This is clearly unphysical and demonstrates the need for bounded noise in therapy models.

% \subsubsection{Numerical Considerations}

% This is not a rare event. Indeed, let us simulate:
% $$
% h + \omega G(t_i)\sqrt{h}
% $$
% Very frequently:
% $$
% h < \omega|G(t_i)|\sqrt{h}
...
% because:
% $$r
% h < 1 \Rightarrow h < \sqrt{h}
% $$

% When using small time steps $h$ in numerical simulations, the stochastic term $\omega G(t_i)\sqrt{h}$ can easily dominate the deterministic term $h$, leading to negative increments even when they should be positive. This numerical artifact further emphasizes the practical importance of using bounded noise processes.

% \subsubsection{Another Subtle Problem}

% Another more subtle problem: the quantity:
% $$
% dt + \omega dW
% $$
% can easily become excessively large, which is not correct (d'Onofrio, 2008).

% Even when the noise doesn't change the sign of a parameter, it can create unrealistically large values that violate physical bounds or make the system unstable. This reinforces the need for properly bounded stochastic processes in realistic models.
