...
\large
$$
    \langle x(t) \rangle = \langle x_0 \rangle e^{\left(rt - \frac{\omega^2 t}{2}\right) + \frac{\omega^2 t}{2}} = \langle x_0 \rangle e^{rt}
$$
\normalsize
...
\small
$$
\partial_t P = - \partial_x  \underbrace{\left[a(x)P(x,t) + \partial_x \left( \frac{b(x)^2}{2}P(x,t) \right) \right]}_{Probability\ current}
$$
\normalsize
...
\begin{exampleblock}[Particle density]
    ...
    $$
    \partial_t \eta = -\partial_x J \qquad \Rightarrow \qquad \partial_t \eta + \partial_x J = 0
    \qquad \xrightarrow{\text{in } \mathbb{R}^2} \qquad \partial_t \eta + \text{div}\, J = 0
    $$
    ...
    $$
    \partial_t \eta(x,t) = -\partial_x\big(f(x)\,\eta(x,t)\big) + \partial_{xx}^2\left(\frac{g^2(x)}{2}\,\eta(x,t)\right)
    $$
    where $\eta$ is normalized so that $\int_{\mathbb{R}} \eta(x,t)\,dx = N = \int_{\mathbb{R}} \theta(x)\,dx$.

    If each particle evolves according to $\dot{x} = f(x) + g(x)\,\xi(t)$, the particle current is:
    $$
    J = f(x)\,\eta(x,t) - \partial_x\left(\frac{g^2(x)}{2}\,\eta(x,t)\right)
    $$
...
\subsection{Noise-Induced Transitions} \label{sec:nit}
