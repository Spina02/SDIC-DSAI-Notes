...
\definitionblock[Machine Learning]{\textbf{Machine Learning} is the science of getting computers to learn something without being explicitly programmed.}
...
The learning process is not written by a human, so we can say that the machine learns \inlinecode{autonomously}.
\begin{itemize}
    \item condsider the universe of functions $F_{X \to Y} = \{f, f:X \to Y\}$
    \item choose the function that does what expected
\end{itemize}

\definitionblock[New Definition]{\textbf{Machine Learning} is the science of getting computers to learn $f_{predict}:X \to Y$ \textbf{autonomously}.}

There has to be some sort of \inlinecode{supervision} facilitating the search of a good $f$ and when it is in the form of some \textbf{examples} and the function should process them correctly we are in the field of \textbf{Supervised Machine Learning}.
\begin{itemize}
...
Examples are pairs of observations and responses ($x,y$), and the set of examples is called \textbf{training set} (or learning set). The \inlinecode{dataset} is a bag of pairs of observations and responses.
...
\definitionblock[Supervised Learning Technique]{
...
\definitionblock[SL Technique as Optimisation]{
...
\inlinecode{Templating $f$} means defining a relationship between data as we do in statistics, specifying it with some coefficients that we can evaluate.
...
Where $m$ is the \inlinecode{model} of how $y$ depends on $x$.
