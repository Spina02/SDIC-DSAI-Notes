...
dA_i \;-\;\delta A_i
$$
è un vettore, in quanto differenza tra due vettori applicati nello stesso punto. Questo $DA_i$, denominato \bfit{Differenziale Assoluto}, permette di introdurre una nuova tipologia di derivata che si comporta come un tensore: la \bfit{derivata covariante}.

Resta ora da determinare $\delta A_i$: se imponiamo che $DA_i$ sia lineare come i dierenziali ordinari, $\delta A_i$ dovrà dipendere linearmente sia dal vettore trasportato $A_i$ che dallo spostamento $du^i$, per cui potremo scrivere:
\vspace{0.4em}
$$
...
La \textbf{derivata covariante per un vettore covariante} $DA_i / \partial u^l$ indicata con $A_{i,l}$ sarà:
\vspace{0.4em}
$$
A_{i,l} = \dfrac{DA_i}{\partial u^l} = \dfrac{\partial A_i}{\partial u^l} - \Gamma^m_{il} A_m
$$

La derivata covariante di un tensore si può ricavare considerando questo come il prodotto di due vettori e richiedendo che essa soddisfi la regola di \emph{Leibniz} per la derivazione di un prodotto. 

Consideriamo allora un tensore di rango due $T_{ik} \equiv A_iB_k$, si ha che:
\vspace{0.4em}
$$
\begin{array}{rcl}
    T_{ik;l} & = & B_kA_{i;l} + A_iB_{k;l} \\[0.8em]
    & = & B_k \left(\dfrac{\partial A_i}{\partial u^l} - \Gamma_{il}^m A_m\right) + A_i \left(\dfrac{\partial B_k}{\partial u^l} - \Gamma_{kl}^m B_m\right) \\[0.8em]
    & = & B_k \dfrac{\partial A_i}{\partial u^l} + A_i \dfrac{\partial B_k}{\partial u^l} - \Gamma_{il}^m A_m B_k - \Gamma_{kl}^m A_i B_m \\[0.8em]
...
Ma $A_{i;k}$ è un tensore, e posso usare il tensore metrico per scriverlo come $A_{i;l} = g_{ik}A_{;l}^k$; se confrontiamo con l'espressione scritta sopra vediamo che $g_{ik;l} = 0$.

\newpage

Usiamo ora la relazione per la derivata covariante di un tensore per esplicitare questo risultato:
\vspace{0.4em}
\begin{alignat}{2}
...
Se facciamo ora (I) + (III) - (II) otteniamo, sfruttando la simmetria degli indici bassi di 
$\Gamma_{il}^m$ e $g_{ik}$:
\vspace{0.4em}
$$
\dfrac{\partial g_{ik}}{\partial u^l} + \dfrac{\partial g_{li}}{\partial u^k} - \dfrac{\partial g_{kl}}{\partial u^i} - \cancel{\Gamma_{il}^m g_{mk}} - \Gamma_{kl}^m g_{im} - \Gamma_{lk}^m g_{mi} - \cancel{\Gamma_{ik}^m g_{lm}} + \cancel{\Gamma_{ki}^m g_{ml}} + \cancel{\Gamma_{li}^m g_{km}} = 0
$$
E semplificando:
$$
...
Moltiplicando ora quest'ultima relazione per $\frac 12 g^{ij}$ otteniamo:
\vspace{0.4em}
$$
\dfrac 12 \left(
...
Ritroviamo quindi la relazione che definisce la connessione affine, verificando così l'assunzione $\Delta_{il}^m \equiv \Gamma^m_{il}$.
...
Sostituendo $\delta A_i = \Gamma^m_{il} A_m du^l$ otteniamo:
\vspace{0.4em}
$$
...
da cui la relazione che esprime la \textbf{derivata covariante per un vettore controvariante}:
\vspace{0.4em}
$$
\dfrac{DB^i}{du^l} = \dfrac{\partial B^i}{\partial u^l} - \Gamma^i_{ml}B^m
$$
che è l'opposto in segno della derivata covariante per un vettore covariante.

\begin{definitionblock}[Regola generale per la derivazione covariante]
...
\begin{tipsblock}[Verifica di un versore]
    possiamo verificare che $v^i$ è un versore, cioè ha modulo unitario, eseguendo il prodotto scalare $v_iv^i$:
    $$
...
\begin{theorem}[Teorema di Stokes]
    Sia $S$ una superficie orientata delimitata da una curva chiusa $C$ orientata positivamente. Per ogni campo vettoriale $\vec{A}$ di classe $C^1$ definito su $S$, vale la relazione:
    vspace{0.4em}
    $$
    \oint_{C} \vec{A} \cdot d\vec{l} = \int_{S} (\nabla \times \vec{A}) \cdot d\vec{S}
    $$
    dove $d\vec{l}$ è l'elemento di linea lungo $C$ e $d\vec{S}$ è l'elemento di superficie orientato su $S$.

    \vspace{-0.6em}
    \rule{\textwidth}{0.4pt}
\end{theorem}

Nella forma tensoriale, l'integrale di linea di un vettore $A_i$ lungo una curva chiusa si scrive come:
...
dove $df^{lm}$ è un tensore che corrisponde alla proiezione dell'elemento di area della superficie sui piani coordinati. Nel nostro caso $A_i du^i \rightarrow \Gamma_{km}^i A_i du^m$, per cui:
...
\vspace{0.4em}
$$
\boxed{R^i_{klm} = \dfrac{\partial \Gamma^i_{km}}{\partial u^l} - \dfrac{\partial \Gamma^i_{kl}}{\partial u^m} + \Gamma^n_{km} \Gamma^i_{nl} - \Gamma^n_{kl} \Gamma^i_{nm}}
$$

\begin{warningblock}[Tensore di Riemann]
    Nella letteratura il tensore di Riemann può essere definito con i segni scambiati rispetto alla definizione qui presentata (il che non cambia le sue proprietà, ma può essere fonte di confusione).
\end{warningblock}
...
\subsection{Proprietà del tensore di curvatura}

Si può dimostrare che $R^i_{klm}$ è l'unico tensore che può essere costruito dal tensore metrico e dalle sue derivate prime e seconde, e che è lineare nelle derivate seconde e quadratico nelle derivate prime. Del tensore metrico si può scrivere la forma totalmente covariante $R_{jklm} = g_{ji}R^i_{klm}$.

Consideriamo il tensore $R^i_{klm}$ come scritto sopra, e mettiamoci nel sistema localmente euclideo in cui, nel punto $u^i$, risulta $\partial g_{ij}/\partial u^k = 0$. Allora le connessioni affini (simboli di Christoffel) sono nulle, e le derivate covarianti si riducono a semplici derivate parziali. In questo sistema:
\vspace{0.4em}
$$
R^i_{klm;j} = \dfrac{\partial R^i_{klm}}{\partial u^j} = \dfrac{\partial^2 \Gamma^i_{km}}{\partial u^j \partial u^l} - \dfrac{\partial^2 \Gamma^i_{kl}}{\partial u^j \partial u^m}
$$

si annullano in $u^i$ le connessioni affini, ma non necessaraimente le loro derivate.

Permutando ciclicamente gli indici $l, m$ e $j$ otteniamo:
\vspace{0.4em}
$$
R^i_{kmj;l} = \dfrac{\partial^2 \Gamma^i_{kj}}{\partial u^l \partial u^m} - \dfrac{\partial^2 \Gamma^i_{km}}{\partial u^l \partial u^j}
\qquad \qquad
R^i_{kjl;m} = \dfrac{\partial^2 \Gamma^i_{kl}}{\partial u^m \partial u^j} - \dfrac{\partial^2 \Gamma^i_{kj}}{\partial u^m \partial u^l}
$$

Sommando le tre relazioni, è facile vedere che si ottiene:
\vspace{0.4em}
$$
\boxed{R^i_{klm;j} + R^i_{kmj;l} + R^i_{kjl;m} = 0}
$$
...
Vediamo le proprietà del tensore di Riemann nella versione completamente covariante $R_{jklm} = g_{ji}R^i_{klm}$:
...
Dal tensore di Riemann, per contrazione, si può ricavare un tensore di rango 2, chiamato \bfit{tensore di Ricci}, definito come:
$$ \boxed{R_{km} = R^i_{kim}} $$

Per le proprietà di antisimmetria del tensore di Riemann, contraendo $i$ ed $m$ in $R^i_{klm}$ otteniamo nuovamente il tensore di Ricci, ma con un segno meno:
$$ R^i_{kli} = -R^i_{kil} = - R_{kl}$$

Il tensore di Ricci è \bfit{simmetrico}:
\vspace{0.3em}
$$
R_{mk} = R^i_{mik} = g^{ir}R_{rmik} = g^{ir}R_{ikrm} = R^r_{krm} = R_{km}
$$

Esso è l'unico tensore simmetrico di rango 2 che si può ottenere da $R^i_{klm}$. Da esso si ricava lo \bfit{scalare di Ricci} o \bfit{scalare di curvatura}:
$$
R = g^{km}R_{km}
$$
...
\begin{itemize}
\item Per $N = 1, \mathcal N = 0$ ed $R_{1111} \equiv 0$ sempre: una curva ha sempre curvatura (intrinseca) nulla, non ho informazioni su come la curva è "embedded" in uno spazio a 2 o più dimensioni. Ciò si traduce nel fatto che non posso capire su quale spazio è adagiata la mia curva, e quindi non posso parlare di curvatura.

\item Per $N = 2, \mathcal N = 1$. C'è un'unica componente indipendente, ad esempio $R_{1221}$:

$$
R_{11} = R^l_{1l1} = g^{il} R_{i1l1} = \underbrace{g^{11}R_{1111}}_{=\,0} + \underbrace{g^{11}R_{1121}}_{=\,0} + \underbrace{g^{21}R_{2111}}_{=\,0} + g^{22}R_{2121} = g^{22}R_{1212}
$$

Dove, per antisimmetria, $R_{ijkl} = - R_{jikl} = - R_{ijlk}$, dunque $R_{1111} = - R_{1111} = 0$, e lo stesso per $R_{1121}$ e $R_{2111}$. Analogamente otteniamo:
$$
R_{12} = R_{21} = -g^{21}R_{1212}, \qquad R_{22} = g^{11}R_{1212}
$$

Ora che abbiamo tutte le componenti, possiamo calcolare lo scalare di Ricci:
...
$$
moltiplichiamo per $g^{il}g^{km}$ e otteniamo:
\vspace{0.3em}
$$
...
La sua divergenza sarà (per la regola della derivazione di un prodotto ed essendo $\delta^l_{j;l} = 0$):
\vspace{0.4em}
$$
R^l_{j;l} - \dfrac 12 \delta_j^l \dfrac{\partial R}{\partial u^l} = R^l_{j;l} - \dfrac 12 \dfrac{\partial R}{\partial u^j} = 0
$$

per quanto visto poco sopra. Il tensore in questione ha quindi divergenza (covariante) nulla. Se passiamo alle componenti covarianti otteniamo:
\vspace{0.4em}
$$
g_{il}R^l_j - \dfrac 1 2 g_{il}\delta^l_j R = R_{ij} - \dfrac 1  g_{ij} R \equiv G_{ij}
$$
dove $G_{ij}$ è detto \bfit{tensore di Einstein}. Questo tensore ha importantissime proprietà: è \textit{simmetrico}, ha \textit{divergenza nulla} e, derivando dal tensore di Riemann, contiene termini \textit{lineari nelle derivate seconde} della metrica e \textit{quadratici nelle sue derivate prime}.

%! Non nel programma (?)

% \section{Il Theorema Egregium}

% In 2 dimensioni il \bfit{Theorema Egregium} di Gauss afferma che la curvatura di Gauss $K$ si può ricavare a partire dal tensor metrico; in particolare, si ha che $K = R_{1212}/g$.

% Diamo qui una giustificazione del Theorema Egregium. Abbiamo visto che, localmente, in un intorno del punto $P$, un elemento di superficie si può scrivere nella forma
% \vspace{0.4em}
% $$
% z = f (x, y) = \dfrac{x^2}{2\rho_1} + \dfrac{y^2}{2\rho_2}
% $$
% che, messo nella forma $x(u, v)$, si può esprimere come $(x \equiv u, y \equiv v)$:
% \vspace{0.4em}
% $$
% \bar x = \left(
%     u, v, \dfrac{u^2}{2\rho_1} + \dfrac{v^2}{2\rho_2},
% \right)
% \qquad \quad
% \bar x_u = \left(
%     1, 0, \dfrac{u}{\rho_1},
...
% Otteniamo:
% $$
% E = \bar x_u \cdot \bar x_u = 1 + \dfrac{u^2}{\rho_1^2}
% \qquad
% F = \bar x_u \cdot \bar x_v = \dfrac{uv}{\rho_1 \rho_2}
% \qquad
% G = \bar x_v \cdot \bar x_v = 1 + \dfrac{v^2}{\rho_2^2}
% $$
% Sostituendo nella prima forma metrica:
% $$
% ds^2 = \left(1 + \dfrac{u^2}{\rho_1^2}\right) du^2 + 2 \dfrac{uv}{\rho_1 \rho_2} du dv + \left(1 + \dfrac{v^2}{\rho_2^2}\right) dv^2
% $$
% otteniamo:
% $$
% g_{ij} = \begin{pmatrix}
%     1 + \frac{u^2}{\rho_1^2} & \frac{uv}{\rho_1 \rho_2} \\
%     \frac{uv}{\rho_1 \rho_2} & 1 + \frac{v^2}{\rho_2^2}
% \end{pmatrix}
% \qquad \to \qquad
% g = \det g_{ij} = 1 + \frac{u^2}{\rho_1^2}+ \frac{v^2}{\rho_2^2}
% $$
% Notimao che, in $P$, $g_{ij} = \begin{pmatrix}
%     1 & 0 \\
%     0 & 1
% \end{pmatrix}$ e $g_{ij;k} = 0$. Ad essere non nulle sono le derivate seconde:
% \vspace{0.4em}
% $$
% g^{ij} = \dfrac 1g
% \begin{pmatrix}
%     1 + \frac{v^2}{\rho_2^2} & -\frac{uv}{\rho_1 \rho_2} \\
%     -\frac{uv}{\rho_1 \rho_2} & 1 + \frac{u^2}{\rho_1^2}
% \end{pmatrix}
% \qquad \qquad
% K|_P = \dfrac{R_{1212}|_P}{g|_P}
% $$
% Calcoliamo ora $R_{1212}$ in $P$, considerando che $g_{12} = 0$ e $g_{11} = 1$:
% \vspace{0.4em}
% $$
% R_{1212} = g_{1k}R^k_{212} = g_{11}R^1_{212} + g_{12}R^2_{212} = R^1_{212}|_P
% $$

% ma le $\Gamma$ contengono le $g_{ij,k}$ che in $P$ sono nulle e rimane:
% \vspace{0.4em}
% $$
% R^1_{212} = \dfrac{\partial \Gamma^1_{22}}{\partial u}\bigg|_P - \dfrac{\partial \Gamma^1_{21}}{\partial v}\bigg|_P
% $$
