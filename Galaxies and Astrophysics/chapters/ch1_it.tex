\chapter{Geometria Differenziale} \label{ch:geometria_differenziale}

La geometria differenziale rappresenta il linguaggio matematico fondamentale per descrivere spazi curvi e le loro proprietà intrinseche. In astrofisica e cosmologia, essa fornisce gli strumenti per analizzare la struttura dello spazio-tempo: tramite tensori, forme differenziali e concetti come curvature e connessioni, si può modellare il comportamento della gravità come descritto dalla relatività generale. 


\section{Geometrie Non Euclidee}
...
\begin{enumerate}
    ...
    \item Data una retta $r$ ed un punto $P$ fuori da essa, per il punto $P$ passa una ed una sola retta parallela ad $r$ (possiamo intenede "parallelo" con \textit{che incontra r solo all'infinito}, in un punto improprio).
...
\begin{tikzpicture}[scale=0.65]
    ...
    \fill (0,1) circle (1.5pt) node[above] {$P$};
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{assets/geometries.png}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.35\textwidth}
        \centering
        \includegraphics[width=0.86\textwidth]{assets/spherical_geometry.png}
...
In questo contesto, i triangoli, noti come triangoli sferici, mostrano una proprietà interessante: la somma dei loro angoli interni supera i 180° ($\alpha + \beta + \gamma > \pi$), un fenomeno noto come eccesso sferico, con l'eccesso proporzionale all'area del triangolo.
\vspace{0.2em}
$$
...
$$
Per costruire un modello della geometria ellittica piana siamo ricorsi all'uso di una sfera (superficie bidimensionale, indicata con $\mathbf S^2$) immersa (\textit{embedded}) in uno spazio euclideo tridimensionale $\mathbf E^3$. 

Notiamo che per rappresentare il 5° postulato abbiamo dovuto ricorrere ad una superficie \textit{curva}, cioè la sfera. Questa \textit{curvatura} deve essere inoltre costante in tutto il piano perché gli altri postulati descrivono lo spazio come omogeneo, e se la curvatura variasse questa proprietà verrebbe meno.

Questo ci porta a comprendere che le diverse geometrie non euclidee possono essere caratterizzate matematicamente attraverso diverse definizioni di distanza. La \bfit{curvatura} dello spazio diventa quindi un elemento fondamentale per distinguere tra i vari tipi di geometrie, ed è data da:
\vspace{0.3em}
$$
k = \dfrac{1}{R^2} \quad \text{dove} \quad R = \text{raggio della superficie}
$$
\vspace{-0.5em}
Si ha quindi:
\begin{itemize}
    \item Curvatura zero $(k = 0)$\phantom{iiiiii costante} → geometria euclidea
    \item Curvatura positiva costante $(k>0)$ \ → geometria ellittica
    \item Curvatura negativa costante $(k<0)$ → geometria iperbolica
\end{itemize}
...
$$
che diverge se uno dei punti tende al bordo. Una rappresentazione parziale di $\mathbf H^2$ in uno spazio euclideo 3-D $\mathbf E^3$ è la cosiddetta \bfit{pseudosfera}, una superficie a curvatura negativa costante, in contrasto con la sfera a curvatura positiva.

\section{Curva piana} \label{sec:curva_piana}

Si può parametrizzare una curva piana nel modo seguente: $\bar x(t) = (x_1(t), x_2(t))$, dove $t$ è un parametro, non necessariamente il tempo; il vettore tangente (velocità) è ${d\bar x}/{\dd t}$. Possiamo definire l'ascissa curvilinea $s(t)$:
$$
O \equiv \bar x(t = 0) \quad P \equiv \bar x(t) \quad ds = |d\bar x| = \bigg|\dfrac {d\bar x}{\dd t}\bigg| \dd t \quad \to \quad s(t) = \int_0^t |d\bar x| = \int_0^t \bigg|\dfrac {d\bar x}{\dd t}\bigg| \dd t
$$
...
una funzione biunivoca (invertibile) che descrive una superficie nello spazio euclideo tridimensionale $\mathbf E^3$. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{assets/surface.png}
...
\frac{d\bar{r}}{\dd t}
\;=\;
\bar{x}_u \;\frac{du}{\dd t}
\;+\;
\bar{x}_v \;\frac{dv}{\dd t}.
$$
...
\begin{definitionblock}[Prima Forma Fondamentale]
    ...
    $$
    una parametrizzazione locale di $M$. In coordinate locali $(u,v)$, ponendo:
    $$
    \bar{x}_u = \frac{\partial \bar{x}}{\partial u},\quad \bar{x}_v = \frac{\partial \bar{x}}{\partial v},
    $$
    l'espressione della \textbf{prima forma fondamentale} e:
    $$
    \dd s^2 = E\,\dd u^2 + 2F\,\dd u\,\dd v + G\,\dd v^2,
    $$
    con
    $$
...
$$
L \;=\; s(b) 
\;=\; \int_a^b \frac{ds}{\dd t}\,\dd t 
\;=\; \int_a^b \bigl|\dot{\bar{r}}(t)\bigr| \,\dd t.
$$
Poiché
$$
\dot{\bar{r}}
...
\;+\;
\bar{x}_v\,\dot{v}
\quad\text{(dove }\dot{u} = \tfrac{du}{\dd t},\; \dot{v} = \tfrac{dv}{\dd t}\text{),}
$$
segue che
\vspace{0.4em}
$$
\biggl(\frac{ds}{\dd t}\biggr)^2
\;=\;
...
\;=\;
\int_{\bar{r}}
\sqrt{\,E\,\dd u^2 \;+\; 2\,F\,\dd u\,\dd v \;+\; G\,\dd v^2\,},
$$
...
Consideriamo il piano in $\mathbf E^3$ descritto da $\bar{x}(u,v)=(u,\,v,\,0)$, con coordinate cartesiane $x=u$ e $y=v$. Allora
$$
\bar{x}_u=(1,0,0), 
\quad
\bar{x}_v=(0,1,0),
...
L
\;=\;
\int_a^b \sqrt{1 + \bigl(f'(x)\bigr)^2}\,\dd x.
$$
\end{exampleblock}
...
\begin{exampleblock}[Sfera in coordinate sferiche]
    ...
    $$
    \dd s^2 = R^2 \cos^2 v\, \dd u^2 + R^2 \dd v^2
    $$
...
$$
\bar v \cdot \bar w = (a\bar x_u + b\bar x_v) \cdot (c\bar x_u + d\bar x_v) = acE + (ad + bc)F + bdG
$$
...
$$
\dd s^2 = g_{11} (\dd u^1)^2 + 2g_{12} \dd u^1 \dd u^2 + g_{22} (\dd u^2)^2 = g_{ij} \dd u^i \dd u^j
$$
...
Nella \emph{notazione di Einstein}, quando un indice compare una volta in alto e una in basso, si sottointende una somma su tutti i valori possibili di tale indice. Per esempio, $g_{ij}\,\dd u^i\,\dd u^j$ significa $\sum_{i,j} g_{ij}\,\dd u^i\,\dd u^j$.
\end{tipsblock}
...
\quad \text{oppure, in forma esplicita}\quad
...
Consideriamo due vettori tangenti $\bar v = \sum_{i} v^i\,\bar x_i$ e $\bar w = \sum_{j} w^j\,\bar x_j$, entrambi definiti in $P$, il loro prodotto scalare è:
$$
\bar v \cdot \bar w 
...
$$
dove $\delta_i^k$ è la \bfit{delta di Kronecker} è definito da: 
$
\quad
\delta_i^k =
\begin{cases}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{assets/area.png}
...
=\; \underbrace{|\bar x_1 \times \bar x_2|\,\Delta u^1\,\Delta u^2
=\; \sqrt{g}\,\Delta u^1\,\Delta u^2}_{\text{identità di Lagrange}}
$$
...
\begin{exampleblock}[Sfera]
    ...
    Calcoliamo l'area:
    $$
    A = \iint_{\Omega} \sqrt{g}\ \dd u\; \dd v = \int_{-\pi}^{\pi} \dd u \int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} R^2 \cos v\; \dd v = \boxed{4 \pi R^2}
    $$
\end{exampleblock}
...
\begin{exampleblock}[Toro]
    ...
    \begin{minipage}[H]{0.6\textwidth}
        $$
        \bar x (u, v) = [R + r \cos u] \cos v, [R + r \cos u] \sin n, r \sin u,
        $$
        $$
        \begin{cases}
            0 < v < 2\pi \\
    ...
    \begin{minipage}[H]{0.4\textwidth}
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.8\textwidth]{assets/toro.png}
        \end{figure}
    \end{minipage}

    Calcoliamo la prima forma fondamentale:
    ...
    \end{cases}
    \quad \Rightarrow \quad
    \sqrt{g} = \sqrt{EG-F^2} = r(R + r\cos u)
    $$
    ...
    $$
    \begin{array}{rl}
    \displaystyle A &= \displaystyle \int_0^{2\pi} \left[ \int_0^{2\pi} r(R + r\cos u) du \right] \dd v \;=\; 2\pi r \left[ \int_0^{2\pi} R\; du + \int_0^{2\pi} r\cos u\; du \right] = \\[1em]
    \displaystyle &= \displaystyle 2\pi r \left[ 2\pi R +r \int_0^{2\pi} \cos u\; du \right] \; 
    \quad \;=\; \quad \boxed{4 \pi^2 R r}
    \end{array}
    $$
