...
\begin{warningblock}[Returning References]
...
Function pointers are useful, but \plaintt{std::function} offers greater flexibility and type safety as it can encapsulate any callable object (including function pointers, lambdas, and function objects) with a compatible signature (see \cref{function-wrappers}).
...
When calling \plaintt{less\_than(3, 5)}, the compiler generates \plaintt{less\_than<int>(int, int)} automatically. You can also specify the template argument explicitly:\newline\plaintt{less\_than<char>('a', 'z')}.
