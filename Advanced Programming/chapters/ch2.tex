...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|}
        \hline
        \textbf{Data Type} & \textbf{Size (Bytes)} \\ \hline
        \plaintt{bool} & 1 \\ 
        (\plaintt{unsigned}) \plaintt{char} & 1 \\
        (\plaintt{unsigned}) \plaintt{short} & 2 \\
        (\plaintt{unsigned}) \plaintt{int} & 4 \\
        (\plaintt{unsigned}) \plaintt{long} & 4 or 8 \\
        (\plaintt{unsigned}) \plaintt{long long} & 8 \\
        \plaintt{float} & 4 \\
        \plaintt{double} & 8 \\
        \plaintt{long double} & 8, 12, or 16 \\ \hline
    \end{tabular}
...
The \textbf{Stack Memory} is used for storing function call frames, local variables, and control information. It is:
\begin{itemize}
    \item Allocated and deallocated automatically.
    \item Suitable for small, short-lived variables.
    \item Limited in size and operates in a last-in, first-out (LIFO) manner.
\end{itemize}

The \textbf{Heap Memory} is used for dynamic memory allocation, which is:
\begin{itemize}
    \item Manually managed through \texttt{new} and \texttt{delete}.
    \item Suitable for objects with varying sizes or extended lifetimes.
    \item Larger in size than the stack but slower due to manual management.
\end{itemize}

\begin{exampleblock}[Stack vs. Heap]
    \begin{codeblock}[language=C++]
int stack_var = 42;          // Stored on the stack.
int* heap_ptr = new int(42); // Stored on the heap.

// Cleanup for heap memory.
delete heap_ptr;
heap_ptr = nullptr;
    \end{codeblock}
\end{exampleblock}
