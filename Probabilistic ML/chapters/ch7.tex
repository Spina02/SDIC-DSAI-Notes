...
\begin{itemize}
    ...
    \item \textbf{Discriminative approach} $p(C_k|x) = f(h(x))$: the goal is to model explicitlythe class posterior (e.g. logistic regression, where $p(C_k|x) = f(w^T\phi (x))$). In this context $f$ is called \textit{activation function}, $f^{-1}$ is called \textit{link function}.
