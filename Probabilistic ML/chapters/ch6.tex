...
\begin{itemize}
    \item \textbf{Linear transformation}: Suppose to have $y = Mx + \eta$ where $x \sim \mathcal{N}(\mu_x, \sum_x), \eta \sim \mathcal{N}(\mu, \sum), x \perp \eta$. Then $y$ is Gaussian, $y \sim \mathcal{N}(M\mu_x + \mu, M\sum_xM^T + \sum)$. This means that Gaussians are closed under linear transformations. 
...
Consider $n$ observations of i.i.d. random variables $\underbar{x} = x_1, \dots, x_n$ ad a family of models $p(x|\theta)$, corresponding to out likelihood distribution, in which we are looking for the model that best fits our data. 
...
Where B is the \textbf{Beta function}. It representa a family of distributions having a domain in $[0,1]$, which can be skewed more toward 0 or 1 by playing with the parameters. It can be showed that 
$$
...
$$
\theta_{ML} = argmax p(\underbar{y}|\underbar{x}, \theta)
$$
...
\begin{definitionblock}[Defign Matrix]
...
\begin{exampleblock}
    ...
    \begin{figure}[H]
        ...
        On the right, the true conditional distri-
        bution $p(t|x)$ in which the green curve
    ...
    \begin{figure}[H]
        ...
        ting the model to the diﬀerent data sets
        varying $\ln(\lambda)$. On the right, the average
        of the fits on the ￿￿￿ datasets (Bishop)}
    \end{figure}
\end{exampleblock}
...
In this scenario, choosing a Gaussian prior and having the Gaussian likelihoodof the linear regression problem, we have an analytical form for our posterios distribution, as we are about to see. 
...
the observation that we have and we using it to make predictions. We
...
\begin{exampleblock}[Online learning]
    ...
    \begin{figure}[H]
        ...
        \caption{Illustration of sequentila Bayesian learning for a sample linear model of the form $f(x, \textbf{w}) = w_0 + w_1 \cdot x$ (Bishop).}
...
distribution for a Bayesian linear regres-
sion model on the dataset shown in
section 6.3. Increasing the number of
data points, the red curve (which rep-
resents the mean of the predictive dis-
tribution) approximates better the sinu-
soidal function and the standard devia-
tion (the shaded region) decreases. Note
...
Thus, the eﬀective number of parameters in Bayesian estimation is
...
when needed. This has the eﬀect of avoiding overfitting and giving a
more correct estimation of the error when doing predictions.

\section{Model Comparison}

Imagine that we are doing linear regression and we pick two diﬀerent
