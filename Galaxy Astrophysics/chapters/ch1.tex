...
% \vspace{-1em}
        ...
        \captionof{figure}{The Azimuthal System \cite{karttunen}}
\end{minipage}
...
\subsubsection{Cosmological redshift from clusters}
...
If the cluster as a whole has no net (bulk) motion, then the individual peculiar velocities of member galaxies approximately cancel out in the cluster frame. As a result, the average observed redshift directly reflects the cosmological redshift:
$$
z_{\text{cluster}} \approx z_{\text{cosm}}
$$
...
A star's space velocity splits into a \bfit{radial velocity} ($v_r$) and a \bfit{tangential velocity} ($v_t$). The tangential velocity leads to the observed proper motion, which can be measured by comparing the star's position on the sky at different times, separated by several years or decades. Let $\alpha$ be the angle swept out by the star on the sky due to its proper motion over a time interval $\Delta t$, $x$ the arc length traveled by the star during that interval, and $d$ the distance from the Sun to the star. Then:

$$
\tan{\dfrac{\alpha}{2}} \sim \dfrac{\alpha}{2} \sim \dfrac {x/2}{d}
...
= \Delta \delta / 1\,\mathrm{year}$, and one along the right ascension direction $\mu_\alpha = \Delta \alpha / 1\,\mathrm{year}$.
...
\begin{minipage}{0.64\textwidth}
    ...
    Therefore, the component in right ascension is written as $\mu^*_{\alpha} = \mu_\alpha \cos \delta$, where the factor of $\cos \delta$ adjusts for this effect.
...
\begin{minipage}{0.64\textwidth}
    The \bfit{luminosity} $L$ of an object is the power emitted by the object throughout a closed surface $\Sigma$ surrounding it, and it is given by:
    $$
    L = \int_{\Sigma} F \, dA
    $$
...
\begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{assets/gal-ext.png}
        \vspace{-0.5em}
    ...
    $$
    where $L(r)$ is the luminosity at distance $r$, $\omega$ is a geometric factor, and $F(r)$ is the observed flux at $r$ taking extinction into account. $F_0$ is the flux at a reference distance (or from a standard star), $R$ is the reference distance, and $\tau$ is the total extinction along the line of sight. 
...
\begin{tipsblock}[Absolute energy distribution and calibration]
    To recover an \bfit{absolute} SED, the measured counts must be placed on an absolute flux scale using \textit{Blackbody} or \textit{spectrophotometric} standards.
    \end{tipsblock}

\subsection{Derived photometric quantities}

Several widely used diagnostics derived from magnitudes and fluxes are collected here.

\subsubsection{Bolometric magnitude and bolometric correction}

The \bfit{bolometric magnitude} measures the brightness integrated over the entire spectrum. Using the SED, the apparent bolometric magnitude is
$$
m_\mathrm{bol} = -2.5 \log_{10}\!\left(\int_0^\infty f_\nu \, d\nu\right) + C_\mathrm{bol},
$$
where $C_\mathrm{bol}$ is a constant that fixes the zero point. The corresponding \bfit{absolute} bolometric magnitude $M_\mathrm{bol}$ is defined at $10$ pc.

\subsubsection{Mass-to-light ratio}

The \bfit{mass-to-light ratio} quantifies how much mass corresponds to a given luminosity in a specific photometric band $X$:
$$
\gamma_{\,_{X}} \equiv \frac{M}{L_X}
$$
It is customary to express $\gamma_{\,_{X}}$ in solar units, i.e., $[M_\odot / L_{\odot, X}]$. The luminosity of an object in band $X$, relative to the solar luminosity in that band, is related to its absolute magnitude by:
$$
\frac{L_X}{L_{\odot, X}} = 10^{-0.4 (M_X - M_{X\odot})}
$$

Consequently, the mass-to-light ratio can be written as:
$$
\gamma_{\,_{X}} = \frac{M / M_\odot}{L_X / L_{\odot, X}} = \left(\frac{M}{M_\odot}\right) \cdot 10^{+0.4 (M_X - M_{X\odot})}
$$

\subsubsection{Surface brightness of extended sources}

The \bfit{surface brightness} quantifies the amount of light per unit apparent area on the sky. It is typically used for extended objects such as galaxies, nebulae, or the Sun's disk.

We use different nomenclature for the different units:
$$
\text{[SB]} = \frac{\text{flux}}{\text{arcsec}^2}
\qquad \text{or} \qquad
\text{[I]} = \frac{L_\odot}{pc^2}
\qquad \text{or} \qquad
\text{[mu]} = \frac{\text{mag}}{\text{arcsec}^2}
$$

(SB) is \textit{independent of distance} in Euclidean geometry (if one neglects extinction and assumes the universe is not expanding).
This property means that near and far galaxies with the same intrinsic luminosity per unit area will appear to have the same observed surface brightness (except for cosmological and extinction effects).

\begin{observationblock}[Isophotal diameters]
For galaxies, a typical size definition is the \bfit{isophotal diameter}, the diameter corresponding to the isophote where $\mu = 25 \ \mathrm{mag\ arcsec^{-2}}$ in the $B$ band, denoted $D_{25}$.
\end{observationblock}

In cosmology, surface brightness is affected by the expansion of the universe (Tolman effect):
$$
I_{\text{obs}} = \frac{I_0}{(1+z)^4}
$$
where $I_0$ is the intrinsic surface brightness and $z$ is the redshift. The observed surface brightness is thus reduced (diluted) by the factor $(1+z)^4$. This effect encompasses the combined impact of photon redshifting, time dilation, and change in apparent size with redshift.