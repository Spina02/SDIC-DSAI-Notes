...
\subsection{Magnitude corrections (Stars)}

\todo{Enhance and complete this section}

The ISM causes interstellar extinction proportional to the frequency of the light. Therefore, we must add a $A_{\lambda}$ correction term to the observed magnitude $m$ to get the intrinsic magnitude $M$:

$$
M = m - 5 \log (d) + 5 - A_{\lambda}
$$

$A_{\lambda}$ is the extinction in magnitudes at the wavelength $\lambda$. In particular we can notice that $A_{B} > A_{V} > A_{R} > A_{I}$.

$$
B-V = (B-V)_0 + \underbrace{A_B - A_V}_{\neq 0}
$$

This ISM response is not dependent on the frequency of the radiations that pass through it. This is calles differential effect.

\begin{observationblock}[Standard stars]
To correct for the ISM effect, we refer to \bfit{standard stars}, which are stars with known magnitudes and colors. These stars are used to calibrate the magnitude system and to correct for the ISM effect.
\end{observationblock}

The quantity $E_{B-V} = A_B - A_V$ is called \bfit{colour excess}. 
