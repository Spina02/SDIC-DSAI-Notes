...
The \textbf{absolute magnitude} $M$ of a source is the apparent magnitude it would have if it were located at a standard distance of 10 parsecs (pc). It is defined by:
$$
m - M = -2.5 \log_{10} \left( \dfrac f F \right)
$$
where $f$ is the observed flux at distance $d$ (in pc) and $F$ is the flux at a standard distance of 10 pc.

Thus, we can obtain the absolute magnitude from the apparent magnitude and the distance, and vice versa.
$$
m = M + 5 \log_{10} (d) - 5,
\qquad \qquad
M = m - 5 \log_{10} (d) + 5
$$
and the \textbf{distance modulus} $m - M$ depends on the distance in parsecs:
$$
m - M = 5 \log_{10} (d) - 5
$$
...
To measure stellar magnitudes with the highest precision, astronomers rely on photoelectric photometers, combined with carefully selected filters that restrict the incoming light to a specific wavelength range. The most widely adopted system is the Johnson photometric system, which originally used three principal filters: $U$ (ultraviolet), $B$ (blue), and $V$ (visual). When we measure brightness through each filter, we obtain the $U$, $B$, and $V$ magnitudes, respectively.

The initial UBV system was later extended to include the $R$ (red) and $I$ (infrared) filters, forming the five-color UBVRI system. Each filter samples a distinct portion of the object's spectral energy distribution, so by comparing magnitudes measured through different filters, we gain direct information about a source's color and temperature.
...
B - V = m_B - m_V
$$
where $m_B$ and $m_V$ are the magnitudes measured through the $B$ and $V$ filters. In practical terms, color indices such as $U - B$ and $B - V$ are commonly reported, while the $V$ magnitude typically serves as the reference for absolute brightness.
...
$$
B - V = C - 2.5 \log_{10} \left( \dfrac{\int_0^\infty d\lambda\, \delta_\lambda(B) f_\lambda}{\int_0^\infty d\lambda\, \delta_\lambda(V) f_\lambda} \right)
$$
...
$$
B - V = M_B - M_V + \cancel{5 \log_{10} d} - \cancel{5} - \cancel{5 \log_{10} d} + \cancel{5} = M_B - M_V
$$
where $M_B$ and $M_V$ are the absolute magnitudes in the $B$ and $V$ bands, respectively. The distance-dependent terms cancel, so the observed color index directly reflects intrinsic properties of the source.

The notation $(B - V)_0$ denotes the \textbf{intrinsic} (reddening-corrected) color index, removing the effects of interstellar extinction and providing a clean diagnostic of the source's physical characteristics such as temperature or composition.